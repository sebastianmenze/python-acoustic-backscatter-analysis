...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.filecounter=-1
        self.filenames = None
        self.folder_with_files = None
        
  
        
        def scan_folder():
            if self.folder_with_files!=None :      
                self.filenames = glob.glob(self.folder_with_files+'\\*.raw')  
                print('found '+str(len(self.filenames)) + ' files')
            else:
                print('No folder')

        def openfolderfunc():
            self.folder_with_files = QtWidgets.QFileDialog.getExistingDirectory(self)
            self.filenames = glob.glob( self.folder_with_files +'\\*.raw')  
            self.filecounter=-1   
            self.df_nasc=pd.DataFrame([])
        ...
        def next_file():
             if len(self.filenames)>0:
                ...
                if self.filecounter>len(self.filenames)-1:
                        ...
                        print('That was it')
                rawfile = self.filenames[self.filecounter]            
                read_raw(rawfile)
