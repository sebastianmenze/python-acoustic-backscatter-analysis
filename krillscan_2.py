...
class Worker_email(QtCore.QThread):
    ...
    def scan_and_send_emails(self):
        ...
        if len(nasc_done)>0:
            ...
            n_files=int(self.config['EMAIL']['email_interval'])
            while len(nasc_done)>=n_files:
                ...
                msg.attach(MIMEText(  str(dict(self.config['GENERAL']))  ,'plain'))

                for fi in files_to_send:      
                                        
                    # ctype, encoding = mimetypes.guess_type(fi)
...
class Worker(QtCore.QThread):
    ...
    def read_raw(self,rawfile):       
        ...
        # for f in raw_freq:
        f=self.config['GENERAL']['transducer_frequency'] 
